  ...
  return (
    <Card padding="p-0" className="overflow-hidden">
      <div className="px-6 py-5 border-b border-beige-200 flex items-center justify-between">
        <div>
          <h3 className="text-sm font-bold text-taupe-900">Carte des signalements</h3>
          <p className="text-xs text-taupe-400 mt-1">Répartition géographique</p>
        </div>
        <div className="flex items-center gap-4">
          {Object.entries(STATUS_CONFIG).map(([key, conf]) => (
            <div key={key} className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: statusColors[key] }} />
              <span className="text-[10px] text-taupe-400 font-medium">{conf.label}</span>
            </div>
          ))}
        </div>
      </div>
            ...
            >
              <Popup>
                <div className="p-1">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{s.territoire_name || s.adresse_approximative}</p>
                  <span className={`inline-block mt-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold ${STATUS_CONFIG[s.status]?.color}`}>
