...
export default function QuestionStep({ question, currentAnswer, onAnswer, onNext, onPrev, stepIndex, totalSteps }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <h2 className="text-3xl sm:text-4xl font-bold text-taupe-900 mb-8 text-center leading-tight">{text}</h2>
      <p className="text-lg text-taupe-600 text-center mb-20 max-w-lg">{description}</p>
      <div className={`grid gap-6 w-full max-w-lg mb-20 ${options.length <= 2 ? "grid-cols-2" : options.length === 3 ? "grid-cols-3" : "grid-cols-2 sm:grid-cols-3"}`}>
        {options.map((opt) => {
          const IconComponent = Icons[opt.icon] || Icons.Circle;
          const selected = isSelected(opt.value);
          return (
            <button key={String(opt.value)} onClick={() => handleSelect(opt.value)}
              className={`flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all duration-200 cursor-pointer ${selected ? "border-canard-600 bg-canard-50 shadow-sm" : "border-beige-200 bg-white hover:border-beige-300 hover:bg-beige-50/50"}`}>
